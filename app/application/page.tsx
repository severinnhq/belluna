...
export default function DigitalMarketingQuiz() {
  ...
  const handleSelection = (value: string) => {
    ...
    if (step.type === "multiple" && step.field === "marketingType") {
      ...
      arr.includes(value)
        ...
        : arr.push(value);
      setFormData({ ...formData, marketingType: arr });
    } else if (step.field) {
      setFormData({ ...formData, [step.field]: value } as any);
    }
  };
  ...
  const renderStep = () => {
    const step = quizSteps[currentStep - 1];
    const value = step.field ? (formData[step.field] as any) : "";
      ...
      return (
        <div className="space-y-4 max-w-md mx-auto">
          <input
            ...
            onChange={(e) =>
              setFormData({ ...formData, fullName: e.target.value } as any)
            }
            className="w-full px-4 py-3 border border-gray-300 rounded-xl"
          />
          <input
